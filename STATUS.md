...
| D121 | 2026-09-21 | **SS-8004 ingest complete for 2021-2026, both chambers:** 899 filings, 875 auto-matched, 49 held; **167 of 207 legislators** have an approved statement and **only 2 current members** lack one. 72 sponsored-travel/expense entries across 54 filings. | Held filings exported to `data/processed/soi_matches_to_review.csv`; `tec_soi apply <csv>` approves rows marked Y. The remaining holds are genuinely ambiguous (a T.J. Hardaway vs G.A. Hardaway; Bruce Carr vs Dale Carr; candidates who never served). |
| D122 | 2026-09-21 | Donor→employer gap-fill keys donor names with PAC-style suffixes stripped ("FEDEX CORPORATION PAC" → "FEDEXCORPORATION"). Insert-only, confidence 85. | Employer names never carry "PAC"; donor names usually do. Without stripping, the registration data matched 3 donors. It fills gaps only — a name-pattern classification already present is never replaced. |
...
| D127 | 2026-09-21 | **SonarQube Cloud scan is scoped by two config files.** `.sonarcloud.properties` (read by Automatic Analysis, the GitHub-import path) and `sonar-project.properties` (read only by a scanner run in CI) both point the scan at `src`, `site/static`, `.github/workflows` and `tests`, Python 3.9 and 3.12. Jinja templates, SQL migrations, docs and data are left out. The project is Public on the free plan; nothing sensitive is tracked in git. | Automatic Analysis ignores `sonar-project.properties` (checked against the SonarSource docs on 2026-09-21), so the requested file on its own would have changed nothing. Jinja templates under the generic HTML analyser and PostgreSQL migrations under the PL/SQL analyser produce noise, not findings. Coverage cannot be imported under Automatic Analysis; a CI scan is the step if that is ever wanted. |
